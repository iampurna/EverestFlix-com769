...
public class VideoService : IVideoService
{
    ...
    public async Task<PagedResponse<VideoSummaryDto>> GetLatestAsync(
        int page,
        int pageSize,
        CancellationToken ct = default)
    {
        ...
        var items = await ProjectSummary(
                query
    ...
    public async Task<PagedResponse<VideoSummaryDto>> SearchAsync(
    ...
    public async Task<Result<VideoDetailDto>> GetByIdAsync(
        int id,
        CancellationToken ct = default)
    {
        ...
        var video = await _db.Videos
            ...
            .Include(v => v.Creator)
            .FirstOrDefaultAsync(
                v => v.Id == id && v.IsPublished,
                ct);

        if (video is null)
        {
            return Result<VideoDetailDto>.Fail(
                ErrorCodes.NotFound,
                "Video not found.");
        }
    ...
    public async Task<Result<VideoDetailDto>> CreateAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            videoUrl = await _storage.SaveAsync(
                request.VideoFile,
                ct);
        }
        ...
        _logger.LogInformation(
            "Video created: {VideoId} by {CreatorId}",
            video.Id,
            creatorId);

        // Reload with Creator navigation populated for the detail DTO.
        var saved = await _db.Videos
            .AsNoTracking()
            .Include(v => v.Creator)
    ...
    public async Task<Result<VideoDetailDto>> UpdateAsync(
        int id,
        ...
        CancellationToken ct = default)
    {
        ...
        if (!isAdmin &&
            video.CreatorId != requestingUserId)
        {
            return Result<bool>.Fail(
                ErrorCodes.Forbidden,
                "You do not own this video.");
        ...
        _logger.LogInformation(
            "Video deleted: {VideoId} by {UserId}",
            id,
            requestingUserId);
    ...
    public async Task<PagedResponse<VideoSummaryDto>> GetCreatorVideosAsync(
        string creatorId,
        int page,
        int pageSize,
        CancellationToken ct = default)
    ...
    public async Task<Result<CreatorDashboardDto>> GetCreatorDashboardAsync(
        string creatorId,
        CancellationToken ct = default)
    {
        ...
        var totalViews = await _db.Videos
            .Where(v => v.CreatorId == creatorId)
            .SumAsync(
                v => (long?)v.ViewCount,
                ct) ?? 0;
        ...
        var ratings = _db.Ratings
            .Where(r =>
                r.Video!.CreatorId == creatorId);

        var averageRating = await ratings.AnyAsync(ct)
        ...
        var totalComments = await _db.Comments
            .CountAsync(
                c => c.Video!.CreatorId == creatorId,
                ct);

        return Result<CreatorDashboardDto>.Success(
            new CreatorDashboardDto
            {
    ...
    private static (int page, int pageSize) Normalize(
        int page,
        int pageSize)
    {
        page = page < 1
    ...
    /// <summary>
    /// EF Core projection used for video summary queries.
    ///
    /// Keeping the CreatorName projection inside the IQueryable allows
    /// EF Core to generate the required SQL JOIN to AspNetUsers rather
    /// than relying on the Creator navigation property being loaded
    /// beforehand.
    /// </summary>
    private static IQueryable<VideoSummaryDto> ProjectSummary(
        IQueryable<Video> query)
    {
        return query.Select(v => new VideoSummaryDto
        {
            Id = v.Id,
            Title = v.Title,
    ...
    private static VideoDetailDto MapDetail(
        Video v) => new()
    {
        Id = v.Id,
        Title = v.Title,
